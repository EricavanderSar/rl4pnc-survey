...
#         changed_topo_vec = np.any(
#             train_batch_new_obs[:, -56:] != train_batch_obs[:, -56:], axis=-1
#         )
#         not_changed_topo_vec = 1 - changed_topo_vec
#         non_terminal_actions = np.any(
#             train_batch_new_obs[:, -56:] != -np.ones_like(train_batch_new_obs[:, -56:]),
#             axis=-1,
#         )
...
#         result["prop_topo_action_change"] = (
#             num_non_zero_actions / train_batch["actions"].shape[0]
#         )
#         result["prop_explicit_do_nothing"] = np.sum(
#             (train_batch_actions == 0) & (non_terminal_actions == 1)
...
#             train_batch_actions[changed_topo_vec], return_counts=True
#         )
#         action_distr_dic = {}
#         for action in range(
#             policy.action_space.n
#         ):  # initalize the dictionary with all actions
#             action_distr_dic[str(action)] = 0
#         for action, count in zip(unique, counts):
#             action_distr_dic[str(action)] = (
#                 count / num_non_zero_actions
#             )  # action distr in percentage

#         # del action_distr_dic[str(0)] # remove the do-nothing action from the action distr
#         result["action_distr"] = action_distr_dic
#         result["num_non_zero_actions_tried"] = sum(
#             [1 for val in action_distr_dic.values() if val > 0]
#         )
...
#         action_distr_dic = result["info"]["learner"]["default_policy"][
#             "custom_metrics"
#         ]["action_distr"]
#         bar_arr = plot_to_array(bar_graph_from_dict(action_distr_dic))
#         self._custom_file_writer = SummaryWriter(
#             self.logdir, flush_secs=30
#         )  # this results in many tf.event files
#         self._custom_file_writer.add_image(
#             "Action_distribution", bar_arr, step, dataformats="HWC"
#         )
