...
def get_action_space(env: BaseEnv,
                     ...
                     incl_dn: bool = False,
                     adjust_shunt: str = "",
                     rho_filter: float = 2.0) -> list[BaseAction]:
    """
        This returns all actions as generated by get_possible_topologies().
    """
    print(f'Initialize "{act_space_name}" action space...')
    ...
    if adjust_shunt == "opt":
        ...
        legal_actions = get_optshunt_actions(env, legal_actions)
    if rho_filter < 2.0:
        legal_actions = apply_rho_filter(env, legal_actions, rho_filter)
...
def apply_rho_filter(env: BaseEnv, actions: list[BaseAction], rho_max: float) -> list[BaseAction]:
    """
    ...
    for idx, act in enumerate(tqdm(actions)):
        ...
        for i in range(n):
            ...
            # try action normal
            obs, reward, done, info = env.step(act)
            max_rhos[i] = obs.rho.max() if obs.rho.max() > 0 else 2
        if max_rhos.mean() < rho_max:
